...
void display_results(int function_id, const int *args,
                     const char *function_names[], const int *num_args) {
  ...
  try {
    ...
    case smarter_section:
      cout << std::boolalpha
           << SmarterSection(args[0], args[1], args[2], args[3]) << endl;
      break;
    case good_dinner:
...
int main() {
  ...
  cout << "Please enter a number to run the function with the corresponding "
          "number: "
       << endl;
  do {
    ...
    if (function_id < 1 || function_id >= num_functions ||
        function_id == exit_the_program) {
      // exit
      break;
    }
