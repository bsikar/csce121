...
# 121 "/usr/include/c++/11/bits/move.h" 3
  ...
  [[__nodiscard__]] constexpr
      typename conditional<__move_if_noexcept_cond<_Tp>::value, const _Tp &,
                           _Tp &&>::type
...
namespace std __attribute__((__visibility__("default"))) {
  ...
  template <typename _Ptr> struct __ptr_traits_elem_1 {};
  ...
  template <typename _Ptr> struct pointer_traits : __ptr_traits_elem<_Ptr> {
  private:
    template <typename _Tp>
...
# 964 "/usr/include/c++/11/bits/stl_iterator.h" 3
  template <typename _Container>
  inline insert_iterator<_Container> inserter(
...
# 989 "/usr/include/c++/11/bits/stl_algobase.h" 3
  ...
  inline constexpr __int128 __size_to_integer(__int128 __n) { return __n; }
...
# 283 "/usr/include/c++/11/bits/char_traits.h" 3
  template <typename _CharT>
  inline __attribute__((__always_inline__)) constexpr bool __constant_string_p(
      const _CharT *__s) {
...
extern int
__sigsetjmp_cancel(struct __cancel_jmp_buf_tag __env[1],
...
extern int pthread_mutexattr_getrobust_np(
    ...
    __attribute__((__nonnull__(1))) __attribute__((
        __deprecated__("pthread_mutexattr_getrobust_np is deprecated, use "
                       "pthread_mutexattr_getrobust")));
...
extern int pthread_mutexattr_setrobust_np(pthread_mutexattr_t *, int) noexcept(
    ...
    __attribute__((__nonnull__(1))) __attribute__((
        __deprecated__("pthread_mutexattr_setrobust_np is deprecated, use "
                       "pthread_mutexattr_setrobust")));
...
# 181 "/usr/include/c++/11/ext/new_allocator.h" 3
    template <typename _Up>
    friend bool operator==(const new_allocator &,
                           const new_allocator<_Up> &) noexcept {
      return true;
    }
...
# 800 "/usr/include/c++/11/istream" 3
  ...
  __attribute__((
      __nonnull__(2),
      __access__(__write_only__, 2))) inline basic_istream<_CharT, _Traits>
      &operator>>(basic_istream<_CharT, _Traits> &__in, _CharT *__s) {
    size_t __n = __builtin_object_size(__s, 0);
    if (__builtin_expect(__n < sizeof(_CharT), false)) {

      do {
        if (__builtin_is_constant_evaluated() && !bool(__n >= sizeof(_CharT)))
          __builtin_unreachable();
      } while (false);
      __in.width(0);
  ...
  __attribute__((
      __nonnull__(2),
      __access__(__write_only__, 2))) inline basic_istream<char, _Traits>
...
public:
  ...
  unsigned int find(const char *str, unsigned int pos = 0) const noexcept;
  unsigned int find(const MyString &str, unsigned int pos = 0) const noexcept;
...
MyString::MyString(const char *str) : data_(nullptr), capacity_(0), size_(0) {
  if (str) {
    size_ = strlen(str);
...
unsigned int MyString::find(const std::string &str,
                            unsigned int pos) const noexcept {}
unsigned int MyString::find(const MyString &str,
                            unsigned int pos) const noexcept {}
unsigned int MyString::find(const char *str, unsigned int pos) const noexcept {}
