...
void outputStatus(const int status, const Player &player, int moves) {
  if (status != STATUS_STAY) {
    cout << "You have moved to row " << player.row << " and column "
         << player.col << endl;
  }
  switch (status) {
  case STATUS_STAY:
  ...
  case STATUS_TREASURE:
    ...
    cout << "You now have " << player.treasure
         << (player.treasure > 1 ? " treasures." : " treasure.") << endl;
    break;
  case STATUS_AMULET:
  ...
  case STATUS_ESCAPE:
    cout << "Congratulations, adventurer! You have escaped the dungeon!"
         << endl;
    cout << "You escaped with " << player.treasure
         << (player.treasure > 1 ? " treasures " : " treasure ");
